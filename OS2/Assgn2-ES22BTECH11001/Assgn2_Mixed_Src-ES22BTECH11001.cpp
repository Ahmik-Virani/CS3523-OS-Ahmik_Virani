...
//These are the global variables, the notation is same as specified in the problem statement
int N, K, C, BT;
...
void multiply(int rowNo)
{
    ...
    for(int i = 0 ; i < N ; i++)
    {
        ...
        for(int j = 0 ; j < N ; j++)
        {
            ans += A[rowNo][j] * A[j][i];	//Computing the value of the required row elements
        }

        OUT[rowNo][i] = ans;			//Storing the value in the corresponding row and column in output matrix (OUT)
    }
}
...
int main()
{
    ...
    //Taking input from the file in the order as specified by the problem statement
    inFile >> N >> K >> C >> BT;
    ...
    /* Main Code Related to thread part */
